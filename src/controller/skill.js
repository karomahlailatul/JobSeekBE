...
const skillController = {
    getPaginationSkill: async (req, res) => {
        ...
        try {
            ...
            if (search === undefined) {
                querysearch = ``;
                totalData = parseInt((await skillModel.selectAll()).rowCount);
            } else {
                querysearch = `where skill.name ilike '%${search}%' `;
                totalData = parseInt((await skillModel.selectAllSearch(querysearch)).rowCount);
            }
    ...
    insertSkill: async (req, res) => {
        try {
            ...
            const { rows: [count], } = await skillModel.countData();
