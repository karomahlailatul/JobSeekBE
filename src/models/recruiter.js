...
const insertRecruiter = (id, users_id, position, company, email, address, phone, logo, description) => {
  return Pool.query(
    `insert into recruiter ( id, users_id , position, company , email, address, phone, logo , description )   values ( '${id}' , '${users_id}' , '${position}', '${company}' , '${email}'  , '${address}'  , '${phone}'  , '${logo}' , '${description}' ) `
  );
};

const insertRecruiterOnRegister = (id, users_id, position, company) => {
  return Pool.query(`insert into recruiter ( id, users_id , position, company )  values ( '${id}' , '${users_id}' , '${position}', '${company}' ) `);
};

const updateRecruiter = (id, users_id, position, company, email, address, phone, logo, description) => {
  return Pool.query(
    `update recruiter set  users_id = '${users_id}' , position = '${position}' , company = '${company}' , email = '${email}' , address = '${address}' , phone = '${phone}' , logo = '${logo}' ,   description = '${description}'   where id = '${id}' `
  );
};

const updateRecruiterNoLogo = (id, users_id, position, company, email, address, phone, description) => {
