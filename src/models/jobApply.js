...
const selectPaginationJobApply_Users_Job_Recuiter_Skill = ({ limit, offset, sortby, sort, querysearch }) => {
    return Pool.query(`select  job_apply.id , job_apply.job_id , job_apply.users_id , job_apply.status , job_apply.created_on , job_apply.updated_on , job.name , job.position ,  job.domicile , job.skill_id , job.description , job.available , job.recuiter_id , job.created_on as job_created_on , job.updated_on as job_updated_on , recuiter.users_id , recuiter.position as recuiter_position  , recuiter.company  , recuiter.email  , recuiter.address  , recuiter.logo  , recuiter.phone  , recuiter.description as recuiter_description , recuiter.created_on as recuiter_created_on , recuiter.updated_on as recuiter_updated_on , skill.name as skill_name , users.name as users_name , users.email as users_email , users.gender , users.phone as users_phone  , users.date_of_birth  , users.picture  , users.job_desk as users_job_desk  , users.domicile as users_domicile  , users.location as users_location , users.created_on as users_created_on   , users.updated_on  as users_updated_on   from job_apply   ${querysearch}  order by ${sortby} ${sort} limit ${limit} offset ${offset} `)
}
...
module.exports = {
    ...
    selectAllSearch,
    selectPagination,
    selectPaginationJobApply_Users_Job_Recuiter_Skill,
    selectJobApply,
    selectUsers,
    selectJob,
    insertJobApply,
